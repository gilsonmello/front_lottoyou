...
Vue.prototype.refreshAuth = function(params) {
  ...
  var access_token = JSON.parse(
    Cookies.get("access_token", { domain }) || null
  );
  access_token = access_token != null ? access_token : null;

  //Token para refresh
  var refresh_token = JSON.parse(
    Cookies.get("refresh_token", { domain }) || null
  );
...
Vue.prototype.getLeaguesOfPackageBySlug = function(slug) {
  return new Promise(function(resolve, reject) {
    let request = axios.create();
    let url = routes.league_packages.find_leagues_by_slug.replace(
      "{slug}",
      slug
    );
    request
