<template>
  ...
  <div class="container" v-else>
    <div class="row">
      <div class="col-lg-12">
        <div class="sub-navigation">
          <router-link
            :to="{ name: 'cartoleando.play', params: { slug: item.package.slug } }"
            class="active show"
          >{{ trans('strings.play_on_the') }} {{ item.package.name }}</router-link>
          <router-link
            :to="{ name: 'cartoleando.leagues', params: { slug: item.package.slug } }"
            class="show"
            id="result-component"
          >{{ trans('strings.leagues') }}</router-link>
        </div>
      </div>
    </div>
      ...
      <div class="col-lg-12 col-12 col-md-12 col-sm-12">
        <h1 class="page-header" style="border: none; margin: 0">
          ...
          <span
            ...
            v-if="auth == null || auth.cartoleando_team == null"
          >Por favor, preencha o formulário para participar do cartoleando</span>
        </h1>
      </div>
    ...
    <form
      @submit.prevent
      id="packages-purchase"
      v-if="auth && auth.cartoleando_team && auth.cartoleando_team.time"
    >
            ...
            <div class="col-lg-12 vcenter-end">
              <div class="cartoleando-show-team-header">
                <img
                  ...
                  :src="auth.cartoleando_team.time.url_camisa_png"
                >
          ...
          <div class="row">
            <div class="col-12 vcenter-end">
              <h4>
                ...
                <br>
                <small>{{ auth.cartoleando_team.time.nome_cartola }}</small>
              </h4>
            </div>
          </div>
        </div>
        ...
        <div class="col-lg-4 no-padding-right">
          <div class="card">
            <div class="card-header">
              <span class="price">
                {{ trans('strings.total_value') }} {{getSystemCurrency.data.symbol}}
                <span
                  class="value"
                  v-if="item.total > 0"
                >{{ totalFormated }}</span>
            ...
            <div class="card-body">
              ...
              <button
                @click="validateFastBuy($event)"
                type="button"
                v-if="loading.paying == false && auth && auth.balance.value >= item.total"
                class="btn-pay-now pull-right btn btn-primary"
              >{{ trans('strings.pay_now') }}</button>
        ...
        <div class="col-lg-8 col-md-8 col-sm-12 col-12">
            ...
            <div class="col-lg-12 col-12 col-sm-12 col-md-12">
              <div class="form-group">
                ...
                <input
                  ...
                  type="text"
                  class="form-control"
                  id="packages-purchase-team-name"
                  aria-describedby="name"
                  :placeholder="trans('strings.team_name')"
                >
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-lg-12 col-12 col-sm-12 col-md-12">
              <div class="form-group">
                ...
                <input
                  name="cartoleiro"
                  v-model="item.cartoleiro"
                  type="text"
                  class="form-control"
                  id="packages-purchase-cartoleiro"
                  aria-describedby="cartoleiro"
                  :placeholder="trans('strings.cartoleiro_name')"
                >
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-lg-12 col-12 col-sm-12 col-md-12">
              <div class="form-group">
                <label
                  for="packages-purchase-email"
                >{{ trans('strings.email') }} cadastrado no site do cartola*</label>
                <input
                  name="email"
                  v-model="item.email"
                  type="email"
                  class="form-control"
                  id="packages-purchase-email"
          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <button
                ...
                type="button"
                v-if="loading.paying == false && auth && auth.balance.value >= item.total && auth.cartoleando_team"
                class="btn-pay-now pull-right btn btn-primary"
              >{{ trans('strings.pay_now') }}</button>
              <button
                v-if="loading.paying"
            ...
            <div class="card-header" data-toggle="collapse" :data-target="'#league_'+league.id">
              ...
              <div class="row vcenter">
                <div class="col-lg-2 col-4 col-sm-2 col-md-2">
                  <img
                    class="img-fluid"
                    :src="league.bg_image_domain+'/'+league.bg_image"
                    :alt="league.name"
                  >
                </div>
                ...
                <div class="col-lg-10 col-8 col-sm-10 col-md-10">
                  ...
                  <span>
                    ...
                    <span
                      v-if="league[league.context] && league[league.context].max_players > league.quantity_teams"
                    >- {{ league[league.context].max_players - league.quantity_teams }} Vagas restantes</span>
                  </span>
                  <br>
        ...
        <div class="modal-content" v-if="item.team">
          ...
          <div class="modal-footer" style="justify-content: center">
            ...
            <button
              ...
              style="width: 40%;"
              type="button"
              @click.prevent="showModalLogin($event)"
            >{{ trans('strings.login') }}</button>
          </div>
        </div>
...
export default {
  methods: {
    ...
    fastBuy() {
      ...
      rq.post(
        ...
        .catch(error => {
          ...
          toastr.error(error.response.data.msg, this.trans("strings.error"));
    ...
    addToCart(el) {
      ...
      if (isvalid && !hasPackage) {
        ...
        addCartoleandoRequest
          .post(routes.carts.add_cartoleandos, {
            purchase: this.item,
            hash: this.item.hash,
            auth: this.auth
          })
        ...
        swal({
          title:
            "O pacote " +
            this.item.package.name +
            " já encontra-se no seu carrinho!",
          showCloseButton: true,
          imageUrl: "/imgs/logo.png",
          imageHeight: 50,
          imageAlt: "Logo lottoyou",
  ...
  computed: {
    ...mapGetters(["auth", "purchase", "loginOptions", "getSystemCurrency"]),
    totalFormated: {
      // getter
      get: function() {
