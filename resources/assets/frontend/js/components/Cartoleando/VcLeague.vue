<template>
  ...
  <div class="container" v-else>
    <div class="sub-navigation">
      <router-link
        :to="{ name: 'cartoleando.play', params: { slug: leaPackage.slug } }"
        class="show"
      >{{ trans('strings.play_on_the') }} {{ leaPackage.name }}</router-link>
      <router-link
        :to="{ name: 'cartoleando.leagues', params: { slug: leaPackage.slug } }"
        class="show active"
        id="result-component"
      >{{ trans('strings.leagues') }} {{ leaPackage.name }}</router-link>
    </div>
    ...
    <div class="row">
      <div class="col-lg-12 col-12 col-md-12 col-sm-12">
        ...
        >{{ trans('strings.leagues') }} {{ leaPackage.name }}</h1>
      </div>
    ...
    <div
      v-else
      class="card"
      v-for="(league, index) in leaPackage.leagues"
      :key="index"
      @click="$router.push({name: 'leagues.show', params: {slug: league.slug}})"
    >
      <div class="card-header">
        ...
        <div class="row vcenter">
          ...
          <div class="col-lg-10 col-8 col-sm-10 col-md-10">
            ...
            <span>{{ trans('strings.league') }} {{ league.modality }}</span>
            <br>
...
import Collapse from "@c/Collapse/Collapse";
import CollapseItem from "@c/Collapse/CollapseItem";
export default {
  components: {
    Collapse,
    CollapseItem
