  ...
  >
    <div class="modal-dialog modal-md">
      <div class="modal-content">
        ...
        <div class="modal-body" v-if="loading.component == true">{{ trans('strings.loading') }}...</div>
        <!-- Modal body -->
            ...
            <div class="row">
              ...
              <div class="col-lg-12">
                <div class="form-group" style="position: relative;">
                  ...
                  >
                  <span class="fa fa-eye view-password" @click.prevent="viewPassword($event)"></span>
                </div>
              </div>
            </div>
            <div class="row">
              <div class="col-lg-12">
                <router-link
                  class="pull-right"
                  :to="{ name: 'users.forgot_password' }"
                >{{ trans('strings.I_forgot_my_password') }}</router-link>
                <router-link
                  style="margin-right: 10px;"
                  class="pull-right"
                  :to="{ name: 'users.register' }"
                >{{ trans('strings.register') }}</router-link>
              </div>
            </div>
            <div class="row">
              <div class="col-lg-12">
    ...
    facebook(newValue, oldValue) {
      if (newValue.status === "connected") {
        window.FB.api(
          ...
          {
            fields:
              "id,email,first_name,last_name,middle_name,name,name_format,picture,short_name",
            debug: "all"
          },
          responseF => {
            ...
            registerRequest
              .post(
                routes.users.create_from_facebook,
                qs.stringify({
  ...
  methods: {
    ...
    viewPassword(event) {
      ...
      if (input.attr("type") == "password") {
        ...
        target.removeClass("fa-eye").addClass("fa-lock");
      } else {
        input.attr("type", "password");
        target.removeClass("fa-lock").addClass("fa-eye");
      }
    ...
    login() {
      ...
      loginRequest
        .post(routes.auth.login, qs.stringify(data))
        .then(response => {
          if (response.status === 200) {
            Cookies.set(
              "access_token",
            ...
            //Fazendo busca do usuário logado, para setar na estrutura de dados
            userRequest
              .get(routes.auth.user, getHeaders())
              .then(response_2 => {
                this.email = "";
                this.password = "";
                this.errors = {};
