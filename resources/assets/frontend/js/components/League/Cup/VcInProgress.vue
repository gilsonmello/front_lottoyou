<template>
  ...
  <div v-else>
      ...
      <div class="col-12 col-lg-6">
        <div class="row no-margin">
          <div class="col-lg-3">
            <div>
              <img
                :src="league.cup.winnerTeam.time.url_escudo_svg"
                class="img-fluid team-shield winner"
              >
            </div>
          </div>
          <div
            class="col-lg-9 vcenter"
            style="flex-direction: column; justify-content: center; align-items: flex-start;"
          >
            <h4 class>CAMPEÃO</h4>
            <p class="no-margin">
              Parabéns {{league.cup.winnerTeam.time.nome_cartola}}!
              <br>Você ganhou a taça, foi o mito da liga e pode tirar onda com a galera. Prepare seu time para novas conquistas.
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-3">
        <div class="row no-margin">
          ...
          <div class="col-lg-12 vcenter-end">
            <div>
              <img
                :src="league.cup.loserTeam.time.url_escudo_svg"
                class="img-fluid team-shield loser"
              >
            </div>
          </div>
          <div class="col-lg-12">
            <h4 class="text-center" style="margin: 10px 0 0 0">{{league.cup.loserTeam.time.nome}}</h4>
            <p class="text-center no-margin">{{league.cup.loserTeam.time.nome_cartola}}</p>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-3">
        <div class="row no-margin">
          ...
          <div class="col-lg-12 vcenter-end">
            <div>
              <img
                :src="league.cup.thirdTeam.time.url_escudo_svg"
                class="img-fluid team-shield loser"
              >
            </div>
          </div>
          <div class="col-lg-12">
            ...
            <p class="text-center no-margin">{{league.cup.thirdTeam.time.nome_cartola}}</p>
          </div>
        </div>
      </div>
    </div>
        ...
        >
          ...
          <div class="carousel-inner" role="listbox">
            <div
              v-for="(step, index) of steps"
              :key="index"
              :class="step.current_step == 1 ? 'carousel-item active' : 'carousel-item'"
            >
              <div class="row">
                <div class="col-12">
              ...
              <vc-step-key
                ...
                :stepKey="stepKey"
                :index="indexKey"
                v-for="(stepKey, indexKey) of step.keys"
                :key="indexKey"
              />
            </div>
          </div>
    ...
    <div class="row" id="conteudo" style>
      <div class="col-lg-12" style="min-height: 50px; background-color: #fff;"></div>
    </div>
  </div>
</template>
...
export default {
  methods: {
    ...
    getSteps() {
      ...
      let url = hostAPI + "/leagues/" + this.league.slug + "/cup/steps";
